...
| 3D | React-Three-Fiber, lazy-loaded, home hero only |
...
- **Every animation lives in `gsap.matchMedia()`** with a `prefers-reduced-motion` branch. No exceptions.
- **Accessibility floor is WCAG 2.1 AA.** Contrast pairs are pre-checked in `theme.css` comments. Never remove `focus-visible`.
- **Case study pages never load Three.js.** The 3D scene is home-hero only, and viewports under 768px get a poster fallback instead of WebGL.
